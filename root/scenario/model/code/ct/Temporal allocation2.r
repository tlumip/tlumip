...
truck_trip_temporal_allocation <- function() {
    ...
    # simulation.
    FN <- str_c(RTP[["ct.properties.folder"]], "/ct-truck-temporal-distributions.csv")
    temporal <- fread(FN)
    ...
    daily_trips$dep_time <- NA
    
    # Process each truck type in turn
    for (t in truck_types) {
        N <- nrow(daily_trips[daily_trips$truck_type==t])
        ...
        daily_trips$dep_time[daily_trips$truck_type==t] <- (hour*100)+minute        
    }
    
    # Replace the existing data with these
    save(daily_trips, file=FN)
    print(str_c("Writing ", nrow(daily_trips), " records to ", FN), quote=FALSE)
    ...
    load(FN)
    truck_types <- unique(long_distance_trips$vehicle_type)
    long_distance_trips$dep_time <- NA
    
    # Process each truck type in turn
    for (t in truck_types) {
        N <- nrow(long_distance_trips[long_distance_trips$vehicle_type==t])
        print(str_c("Sampling departure times for ", N, " long-distance ", t,
            " trucks"), quote=FALSE)
        hour <- sample(temporal$hour[temporal$truck_type==t], N,
            prob=temporal$share[temporal$truck_type==t], replace=TRUE)
        minute <- sample(0:59, N, replace=TRUE)
        long_distance_trips$dep_time[long_distance_trips$vehicle_type==t] <-
            (hour*100)+minute        
    }
...
TA <- truck_trip_temporal_allocation()