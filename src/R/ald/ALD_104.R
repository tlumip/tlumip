...
##/
 # Read in the command line arguments to identify the code path, data path and year
 # @return codePath - the path to the ALD code directory
 # @return dataPath - the path to the model scenario data
 # @return beginYear - the year corresponding to the start of the period
 # @return endYear - the year corresponding to the end of the period
##/
codeVersion <- "104"
codePath <- sub("-", "", commandArgs()[4])
dataPath <- sub("-", "", commandArgs()[5])
beginYear <- as.numeric(sub("-", "", commandArgs()[6]))
endYear <- beginYear + 1

##/
 # Identify the locations/names where the input files reside and where the outputs will be written
 # @param dataPath - see ALD.R
 # @param beginYear - see ALD.R
 # @param endYear - see ALD.R
 # @return EDDirectory
 # @return PIPrevDirectory
 # @return ALDPrevDirectory
 # @return ALDCurrDirectory
##/
EDDirectory <- paste(dataPath, "t0", "/ED", sep="")
PIBeginDirectory <- paste(dataPath, "t", beginYear, "/PI", sep="")
ALDStartDirectory <- paste(dataPath, "/t0", "/ALD", sep="")
ALDBeginDirectory <- paste(dataPath, "t", beginYear, "/ALD", sep="")
ALDEndDirectory <- paste(dataPath, "t", endYear, "/ALD", sep="")
workingDirectory <- paste(dataPath, "t", endYear, "/ALD", sep="")
...
##/
fieldNames <- c("alphaZone", "betaZone", "state", "county", "fips", "puma1pct", "puma5pct", "gridAcres", "luIntensity")
fieldTypes <- c("character", "character", "character", "character", "character", "character", "character", "numeric", "character")
...
##/
fieldNames <- c("year", "res", "nres")
nivqData <- read.table(input$nivqData, header=T, sep=",")
nivq <- nivqData[endYear, 2:3] # Note: assumes that the first line is year 0
...
compatibilityKey <- c(NP=0, VL=0.01, L=0.2, M=0.5, H=0.8, VH=1)
...
##/
funs$allocateFloorProd <- function(nivq, occupied.bf, prevq.af, Lq, Bq1, Bq2, asc.f){
    # Make the scalar Bq1 and Bq2 coefficients into vectors the length of the number of floorspace types
    Bq1.f <- rep(Bq1, length(asc.f))
    Bq2.f <- rep(Bq2, length(asc.f))
    # Calculate average vacancy rates
    tvq.f <- colSums(prevq.af)
    occupied.f <- colSums(occupied.bf)
    vacancy.f <- (tvq.f - occupied.f) / tvq.f
...
##/
funs$calcFloorCapacity <- function(compat.fz, zoning.az, far.fz, floorDef){
    # Calculate the proportion of the land area of each zoning category allocated to each floorspace type
    expCompatUtil <- exp(Lp * compat.fz)
    expCompatUtil[compat.fz == 0] <- 0
    landProp.fz <- expCompatUtil %row/% colSums(expCompatUtil)
    landProp.fz[is.nan(landProp.fz)] <- 0
...
##/
 ...
 # @param price.bf - matrix of floorspace prices by beta zone and floorspace type
 # @param alphaBeta - a data frame of correspondence between alpha and beta zones
...
##/
 ...
 # @return nivq.f$nres - a vector of square footage of new floorspace by floorspace type for the model area
 ##/
residentialFloorspace <- allocateFloorProd(nivq$res, occupied.bf$res, quant.af$res, rLq, rBq1, rBq2, asc$res)
...
##/
 ...
 # @param  price.bf$nres (an element of vars)
 ...
 # @return niq.f$nres - a vector of square footage of new floorspace by floorspace type for the model area
 ##/
nonresidentialFloorspace <- allocateFloorProd(nivq$nres, price.bf$nres, quant.af$nres, nLq, nBq1, nBq2, asc$nres)
...
##/
floorspaceCapacities <- calcFloorCapacity(compat.fz, zoning.az, far.fz, floorDef)
...
##/
 ...
 # @param totQ.af
 # @param ALDEndDirectory
 # @return "FloorspaceInventory.csv" - matrix results in the ALDCurrDirectory
##/
# Write in ALD matrix format
write.table(totQ.af, paste(ALDEndDirectory, "FloorspaceInventory.csv", sep="/"), row.names=TRUE, col.names=TRUE, sep=",")
...
write.table(floorOut, paste(ALDEndDirectory, "FloorspaceDiagnostics.csv", sep="/"), row.names=TRUE, col.names=TRUE, sep=",")
rm(floorOut)
...
##/
write.table(floorspaceCapacities$res, paste(ALDEndDirectory, "ResidentialCapacity.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(floorspaceCapacities$nres, paste(ALDEndDirectory, "NonresidentialCapacity.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(floorspaceCapacities$floorspaceProp, paste(ALDEndDirectory, "FloorspaceProportionsByZoning.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(floorspaceCapacities$landAllocations, paste(ALDEndDirectory, "LandAllocations.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
...
##/
write.table(resDQ$A.af, paste(ALDEndDirectory, "ResidentialDecreaseUtilities.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(resDQ$DQ.af, paste(ALDEndDirectory, "ResidentialDecrease.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(nresDQ$A.af, paste(ALDEndDirectory, "NonresidentialDecreaseUtilities.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(nresDQ$DQ.af, paste(ALDEndDirectory, "NonresidentialDecrease.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
...
##/
write.table(resQNew$prevDQ.af, paste(ALDEndDirectory, "ResQuantitiesAfterDecrease.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(nresQNew$prevDQ.af, paste(ALDEndDirectory, "NonresQuantitiesAfterDecrease.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(resQNew$propCap.af, paste(ALDEndDirectory, "ResCapacityUtilization.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(nresQNew$propCap.af, paste(ALDEndDirectory, "NonresCapacityUtilization.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(resQNew$B.af, paste(ALDEndDirectory, "ResIncreaseUtilities.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(nresQNew$B.af, paste(ALDEndDirectory, "NonresIncreaseUtilities.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(resQNew$IQ.af, paste(ALDEndDirectory, "ResidentialIncreases.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
write.table(nresQNew$IQ.af, paste(ALDEndDirectory, "NonresidentialIncreases.csv", sep="/"), 
            row.names=TRUE, col.names=TRUE, sep=",")
...
##/
 ...
 # @quant.af$agfor
 # @param ALDEndDirectory
 # @return "FloorspaceI.csv" - flat form results in the ALDEndDirectory
##/
...
write.table(totQOut, paste(ALDEndDirectory, "FloorspaceI.csv", sep="/"), row.names=FALSE, col.names=TRUE, sep=",")
...
write.table(IQOut, paste(ALDEndDirectory, "Increments.csv", sep="/"), row.names=FALSE, col.names=TRUE, sep=",")
...
setwd(workingDirectory) # To save the .Rdata file to the ALD directory for the end period
