...
specialPIFile                   <-      paste(PIBeginDirectory, "ExchangeResultsI.csv", sep="/")
ordinaryPIFile                  <-      paste(PIBeginDirectory, "ExchangeResults.csv", sep="/")
if(beginYear == "0"){
    if(file.exists(specialPIFile))
        input$exchangeResults   <-      specialPIFile else
        input$exchangeResults   <-      ordinaryPIFile
    }
if(beginYear != "0"){
    input$exchangeResults       <-      ordinaryPIFile
    }
input$floorQuantities           <-      paste(ALDBeginDirectory, "FloorspaceInventory.csv", sep="/")
