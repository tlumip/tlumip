...
class A2B(object):
    def __init__(self, a, b, st, ct, fips, mpoModel, puma00, puma90):
        self.alpha = int(a)
        self.beta = int(b)
        self.state = st
        self.county = ct
...
class Alpha2Beta(dict):
    ...
    def readA2B(self, a2bFileName):
        ...
        for line in f:
            if k > 0:
                a, b, st, ct, fips, puma1, puma5, acres, lu, mpoCalib, mpoModel, act, aggAct, stPuma00, puma00, puma90, stFips, cntyFips = [ i.strip() for i in line.split(",") ]
                a2b[int(a)] = A2B(a, b, st, ct, fips, mpoModel, puma00, puma90)
            k += 1
...
class RmseReport(object):
    ...
    def writeReportToFile(self, title, period, outputFileName):
...
def rmseReports(year, period, outputFilename, a2bFileName, countsFile):
    ...
    for i, attribute in enumerate([ 'linkType', 'volRange', 'mpoAreas' ]):
        ...
        report.writeReportToFile( titles[i], period, outputFilename )
...
rmseReports( year, period, filename, a2bFileName, countsFile )
