...
/**
 * PIModel essentially has to: 1) Call migrationAndAllocation for each
 * ProductionActivity for a given time step. 2) if some Production Activities
 * are modelled as being in equilibrium with each other, PIModel will need to
...
public class PIModel extends ModelComponent {
    ...
    private double stepSize = .5; //actual stepSize that will be used by PI,
    ...
    /*
     * This constructor is only called by AO when running PI monolithically
     * (which will not be called very often if ever). The properties are set by
     * a call to the setApplicationResourceBundle method which is defined in the
     * Model Component class and then PIModel is started by a call to the
     * 'startModel(timeInterval)' method which calls PIControl
     */
    public PIModel() {
    ...
    /*
     * This constructor is used by PIControl and by PIDAF - The pi.properties
     ...
     * we need to pass in the ResourceBundle to PIModel from within the
     * PIServerTask. Because there may be more than 1 pi.properties file over
     ...
     * pi.properties file based on the timeInterval and will write it's absolute
     * path location into a file called 'RunParameters.txt' where it will be
     * read in by PIServerTask in it's 'onStart( ) method and will subsequently
     * be passed to PIModel during the 'new PIModel(rb)' call.
     */
    ...
    public boolean calculateCompositeBuyAndSellUtilities() {
        ...
        while (allOfUs.hasNext()) {
            ...
            //  just for debugging create storage to save composite utilities
            // calculated
            ...
            //   logger.info("CONSTRUCTION in zone
            // "+AbstractTAZ.getZone(1).getZoneUserNumber()+" buying utility
            // calculated is "+compUtils[0][1]);
            //    logger.info("CONSTRUCTION in zone
            // "+AbstractTAZ.getZone(1).getZoneUserNumber()+" selling utility
            // calculated is "+compUtils[1][1]);
            //}
    ...
    /*
     * PI-DAF Method: The following method will be called when PI is distributed
     * and replaces the "calculateCompositeBuyAndSellUtilities() method, that is
     * used when PI is running on a single machine.
     */
    public double[][] calculateCompositeBuyAndSellUtilities(String name) {
        double[][] compUtils = null; //compUtils[0][] = buy utils,
                                     // compUtils[1][]= sell utils
    ...
    public boolean allocateQuantitiesToFlowsAndExchangesWithFullDerivatives() {
        ...
        while (allComms.hasNext()) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Finished allocating commodity " + count + " in " + (System.currentTimeMillis() - activityStartTime) / 1000.0
                        + " seconds");
            }
            count++;
        }
    ...
    /*
     * PI-DAF Method: allocates the total production/consumption amount in a
     * zone of a single commodity to the exchange zone as Buying and Selling
     * quantities (Bc,z,k and Sc,z,k).
     *  
     */
    public Commodity allocateQuantitiesToFlowsAndExchanges(String name, double[] TCs, double[] TPs, double[] dTCs, double[] dTPs, double[] prices) {
        boolean nanPresent = false;
        Commodity c = Commodity.retrieveCommodity(name);
        c.clearAllExchangeQuantities();//iterates through the exchange objects
                                       // inside the commodity
    ...
    /*
     * PI-DAF Method: This method (called by the SDWorkTask) calculates the
     * imports and exports in each exchange zone and returns the surplus and the
     * derivative of the particular commodity in each exchange zone.
     */
    public double[][] calculateSurplusAndDerivatives(String name) {
        ...
        //       long startTime = System.currentTimeMillis();
        Commodity c = Commodity.retrieveCommodity(name);
        double[][] sAndD = new double[2][c.getBuyingTazZUtilities().values().size()]; //sAndD[0][]=
                                                                                      // surplus
                                                                                      // in
                                                                                      // each
                                                                                      // exchange,
    ...
    public double calculateMeritMeasureWithLogging() throws OverflowException {
        ...
        while (commodities.hasNext()) {
            ...
            if (maxExchange != null) {
                logger
                        .info("\t maxSurp: " + maxSurplusSigned + " in " + maxExchange + " planning DP from " + maxExchange.getPrice() /*
                                                                                                                                        * + "
                                                                                                                                        * to " +
    ...
    //        DenseMatrix firstDerivatives = new
    // DenseMatrix(numberOfExchanges,numberOfExchanges);
    //
    //        
    //        // populate storage of full derivatives
    //        Iterator actIt =
    // AggregateActivity.getAllProductionActivities().iterator();
    //        while (actIt.hasNext()) {
    ...
    //                try {
    //                    pl= new
    // DenseVector(activity.logitModelOfZonePossibilities.getChoiceProbabilities());
    //                    fpl = new
    // DenseMatrix(activity.logitModelOfZonePossibilities.choiceProbabilityDerivatives());
    //                    
    ...
    //                    AggregateDistribution l = (AggregateDistribution)
    // activity.logitModelOfZonePossibilities.alternativeAt(location);
    //                    DenseVector dul1bydprice = new
    // DenseVector(l.calculateLocationUtilityDerivatives(exchangeNumbering));
    ...
    //                }
    //                for (int location =0;location<pl.size();location++) {
    //                    VectorI dThisLocationByPrices = new
    // DenseVector(dLocationByPrice.sizeOfColumns());
    ...
    //        }
    //
    //        // now add the components associated with the derivative of exchange
    // location choice
    ...
    //                try {
    //                    Matrix bzuDerivatives = new
    // DenseMatrix(bzu.myFlows.getChoiceDerivatives());
    //                    ContiguousVector diagonal = new
    // ContiguousVector(bzuDerivatives.sizeOfColumns());
    //                    diagonal.setElements(-bzu.getQuantity()*c.getBuyingUtilityPriceCoefficient());
    //                    SparseMatrix diagonalMatrix = new
    // SparseMatrix(bzuDerivatives.sizeOfColumns(),
    // bzuDerivatives.sizeOfRows());
    //                    diagonalMatrix.setDiagonal(diagonal);
    //                    bzuDerivatives = a.multiply(bzuDerivatives,diagonalMatrix);
    //                    
    //                    Matrix szuDerivatives = new
    // DenseMatrix(szu.myFlows.getChoiceDerivatives());
    ...
    //                    if (exchangeChoiceDerivatives ==null) exchangeChoiceDerivatives =
    // szuDerivatives;
    //                    else exchangeChoiceDerivatives =
    // a.add(exchangeChoiceDerivatives,bzuDerivatives);
    //                    exchangeChoiceDerivatives =
    // a.add(exchangeChoiceDerivatives,szuDerivatives);
    ...
    //               
    //            }
    //            // now finished with the commodity, need to add this submatrix to the big
    // matrix
    //            Iterator exIt = c.getAllExchanges().iterator();
    //            int exNumber = 0;
     ...
     * pi.properties file has been found in the tn subdirectories and has been
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    /***************************************************************************
     * ALL METHODS BELOW THIS LINE HAVE BEEN COMMENTED OUT BECAUSE THEY ARE NOT
     * BEING USED (AS OF 3/18/04) OR THEY HAVE BEEN MOVED TO OTHER CLASSES.
     **************************************************************************/
    ...
    //        allocateCommoditiesAndActivitiesBasedOnInitialPrices(); //will throw a
    // runtime exception if initial prices are
    //                                                                //unreasonable.
    //
    ...
    //            throw new RuntimeException("Overflow with initial prices --
    // terminating");
    //        }
    //        logger.info("New prices have been defined. Time in seconds:
    // "+(System.currentTimeMillis()-startTime)/1000.0);
    //
    //        logger.info("##### progress ##### entering do-loop in
    // iterateAggregateFactors");
    //        int numConverged = 0;
    //        do {
    //            boolean nanPresent = false;
    //            iterations++;
    ...
    //            logger.info("Finished allocation commodities. Time in seconds: "+
    // (System.currentTimeMillis()-allocationStartTime)/1000.0);
    //
    //            // figure out whether we've gotten anywhere, and figure out another
    // tentative step too.
    ...
    //            newPrices = calculateNewPrices(stepSize);
    //            logger.info("Merit measures: old: " + oldMeritMeasure + " new: " +
    // newMeritMeasure);
    //            if (nanPresent) {
    ...
    //                nanPresent = false;
    //            } else if (newMeritMeasure > oldMeritMeasure && newMeritMeasure /
    // oldMeritMeasure > 1.0000000001 && stepSize > getMinimumStepSize()) {
    //                // going the wrong way...
    //                // lets not be so agressive
    //                stepSize *= decreaseStepSizeMultiplier;
    //                logger.info("** decreasing step size to " + stepSize + " abandoning
    // planned price changes");
    //                // abandon the tentative price change, and, as well, back up the step
    // size for our last price change
    //                newPrices =
    // StepPartwayBackBetweenTwoOtherPrices(oldPrices,currentPrices,decreaseStepSizeMultiplier);
    //                // back everything else up
    //                newMeritMeasure = oldMeritMeasure;
    //                currentPrices = oldPrices;
    //            } else {
    //                if (newMeritMeasure / oldMeritMeasure > 0.999) numConverged++; else
    // numConverged = 0;
    //                stepSize *= increaseStepSizeMultiplier;
    //                if (numConverged >= 300) logger.info("no progress -- stopping and
    // claiming victory");
    ...
    // "+(System.currentTimeMillis()-startTime)/1000.0);
    //
    //            logger.info("Finishing iteration "+iterations+" Time in seconds:
    // "+(System.currentTimeMillis()-loopStartTime)/1000.0);
    //        } while (numConverged < 300 && newMeritMeasure > converged && iterations
    // < maxIterations);
    //        logger.info("##### progress ##### exiting do-loop in
    // iterateAggregateFactors");
    //        if (iterations >= maxIterations) logger.warning("stopping because max
    // iterations reached");
    //    }
    //public void runComponent() {
    // message #1.1 to
    // disaggregateFactors:com.pb.despair.pi.DisaggregateActivity
    ...
    //        increaseStepSizeMultiplier =
    // Double.parseDouble(props.getProperty("increase.stepsize"));
    //        decreaseStepSizeMultiplier =
    // Double.parseDouble(props.getProperty("decrease.stepsize"));
    ...
    //        maxIterations =
    // Integer.parseInt(props.getProperty("maximum.iterations"));
    ...
    // 'Commodity.fixPricesAndConditionsForAllCommodities'");
    //            long startTime = System.currentTimeMillis();
    //            Commodity.fixPricesAndConditionsForAllCommodities();//calculate all CUBuy
    // and CUSell
    //            logger.info("'Commodity.fixPricesAndConditionsForAllCommodities' is
    // complete. Time in seconds:
    // "+((System.currentTimeMillis()-startTime)/1000.0));
    //
    //            logger.info("Entering 'resetCommoditiesBoughtAndSoldQuantities'");
    //            startTime = System.currentTimeMillis();
    //            resetCommodityBoughtAndSoldQuantities();//setting sell qty and buy qty to
    // 0 and setting derivative to 0
    //                                                        // in the CommodityZUtility objects (all comms, all zones)
    //            logger.info("'resetCommoditiesBoughtAndSoldQuantities' is complete. Time
    // in seconds: "+((System.currentTimeMillis()-startTime)/1000.0));
    //
    //            logger.info("Beginning Activity Iteration: calling
    // 'migrationAndAllocation' for each activity");
    //            startTime = System.currentTimeMillis();
    //            int count=1;
    //            Iterator pit =
    ...
    //                long activityStartTime = System.currentTimeMillis();
    //                aa.migrationAndAllocation(1.0, 0, 0); //allocates activities to zones
    // (timeStep=1, inMigration=0, outMigration=0
    //                logger.info("Finished activity "+ count+ " in "+
    ...
    //            logger.info("All activity calculations are complete. Time in seconds:
    // "+((System.currentTimeMillis()-startTime)/1000.0));
    //
    //            logger.info("Beginning Commodity Iteration: calling
    // 'allocateQuantitiesToFlowsAndExchanges' for each commodity");
    //            startTime = System.currentTimeMillis();
    //            allocateQuantitiesToFlowsAndExchanges(); //calculates the buy and sell
    // qtys (Sc,z,k & Bc,z,k)
    //                                                    //flowing from the zones to the exchanges (+sell, -buy(flow is really
    //                                                    //from exchanges to zones.))
    //            logger.info("All commodity calculations are complete. Time in seconds:
    // "+((System.currentTimeMillis()-startTime)/1000.0));
    //
    //       } catch (OverflowException e1) {
    //                logger.severe("Overflow when running from starting prices -- change
    // starting prices and retry");
    //                e1.printStackTrace();
    //                throw new RuntimeException("Overflow when running from starting prices --
    // change starting prices and retry");
    //       }
    //       Commodity.unfixPricesAndConditionsForAllCommodities();
    //    }
