...
public class Patterns implements Cloneable{
    ...
    static boolean debug = false;
     ...
     public static TableDataSet loadTableDataSet(ResourceBundle rb, String fileName){
        if(debug) logger.info("loadTableSet is getting the following property from the rb " + fileName);
        try {
           CSVFileReader reader = new CSVFileReader();
           String file = ResourceUtil.getProperty(rb, fileName);
           if(debug) logger.info("loadTableDataSet is trying to read file: "+ file);
    ...
    public void readData(ResourceBundle rb, String fileName){
       ...
       if(debug) logger.fine("Success reading table: "+patternsTableName);
 ...
 */    }
 ...
 /*    public void addPatternsToJDataStore(DataManager dm){

          logger.fine("Adding table "+patternsTableName);
         //DataManager dm = new DataManager();  //Create a data manager, connect to default data-store

          if( dm.tableExists(patternsTableName))
               dm.deleteTable(patternsTableName);          
          
         dm.loadTable(patternsTableName,patternsFileName);
          //dm.closeStore();     
     };     
     
     public void readPatternsFromJDataStore(DataManager dm){        
          
          logger.fine("Getting table: "+patternsTableName);
         //DataManager dm = new DataManager();  //Create a data manager, connect to default data-store

        TableDataSet table = dm.getTableDataSet(patternsTableName);
        try {
            while (table.inBounds()) {

                    Pattern thisPattern = new Pattern(table.getString("patternString"));

//                 System.out.println("Reading pattern :"+thisPattern.dayPattern.toString());

                      patterns.add(thisPattern);    
                table.next();                                            
            }                                                            
        } catch (com.borland.dx.dataset.DataSetException e) {            
            logger.severe("Error: readWeekdayPatternsFromJDataStore()");                   
            e.printStackTrace();          
            System.exit(1);                                
        }                                                                
          logger.fine("Success reading table: "+patternsTableName);
